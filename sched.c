...
{
    int cnt = 0;
    sched_delete_ready_queues();scheduler_status = SCH_STOPPED;
    scheduler_method = SCH_MLFQ;num_of_ready_queues = numOfLevels;
    env_ready_queues = kmalloc(num_of_ready_queues * sizeof(struct Env_Queue));
    while (cnt < num_of_ready_queues) {
        init_queue(&(env_ready_queues[cnt]));
        cnt++;
    }
    quantums = kmalloc(num_of_ready_queues * sizeof(uint32));
    cnt = 0;
    while (cnt < num_of_ready_queues) {
        quantums[cnt] = quantumOfEachLevel[cnt];
        cnt++;
    }
...
struct Env* fos_scheduler_MLFQ() {
    ...
    int bol = 0, cnt = 0;
    while (cnt < num_of_ready_queues) {
        if (queue_size(&(env_ready_queues[cnt])) > 0) {
            bol = 1;
            MLFQCurLevel = cnt + 1;
            break;
        }
    ...
    if (bol == 1) {
        ...
        Nxt_ENV = dequeue(&(env_ready_queues[MLFQCurLevel - 1]));
    }
    return Nxt_ENV;
}
