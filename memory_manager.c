...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* created_TP = (void *)kmalloc(PAGE_SIZE);       // created_TP is created page table
	memset((uint32*)created_TP, 0, PAGE_SIZE);
	uint32 page_PHaddress = kheap_physical_address((uint32)created_TP);
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(page_PHaddress, PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
	tlbflush();

	return (void *)created_TP;
}
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int end = ROUNDUP(size, PAGE_SIZE);
	int start=0;
	while(start < end)
	{
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 end = ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;
	...
	uint32 * wsPageDirect = e->env_page_directory;
	while(idx < wsSize)
	{
		uint32 wsva=env_page_ws_get_virtual_address(e,idx);
		if(wsva >= start){
			if(wsva < (start+ROUNDUP(size,PAGE_SIZE)))
			{
				unmap_frame(wsPageDirect,(void*)wsva);
	...
	for(int i = 0; i < end; i++) {
		wspt=NULL;
		int checkoFree = 1;
		get_page_table(wsPageDirect, (void*)start, &wspt);
		if(wspt != NULL){
			int entryIdx = 0;
			while(entryIdx < PenTEN)
			{
				if(wspt[entryIdx] != 0)
				{
					checkoFree = 0;
